...
let env = {};
try { env = parseEnv(envPath); } catch (e) { console.warn('Could not read .env:', e.message); }
...
(async () => {
  ...
  const postData = JSON.stringify({ title: 'Test Upload', resumeText: 'Sample resume text for testing upload endpoint.' });
